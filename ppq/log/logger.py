...
class LEVEL(Enum):
    ...
    def convert(level: str):
        ...
        if level == "debug":
            ...
            raise ValueError(
                "the lowercase of param level should be one of debug/info/warning/error"
...
class Handler:
    def __init__(self, file_name: str = None, level: LEVEL = LEVEL.INFO) -> None:
        ...
        self._level = level
        self._fd = (
            sys.stdout if file_name is None else open(file_name, "w+", encoding="utf-8")
        )
...
class NaiveLogger(object):
    ...
    def __init__(
        self, create_key: object, name: str, level: LEVEL, file_name: str
    ) -> None:
        assert (
            create_key == NaiveLogger.__create_key
    ...
    def set_level(self, level: Union[str, LEVEL]):
        ...
        assert isinstance(
            level, LEVEL
        ), "level should be given by either str or LEVEL instances"
        self._level = level
    ...
    @classmethod
    def get_logger(
        cls, name: str, level: Union[str, LEVEL] = LEVEL.INFO, file_name: str = None
    ):
