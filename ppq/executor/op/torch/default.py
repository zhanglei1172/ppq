    ...
    if ndim == 5:
        ...
        if isinstance(onnx_pads, list) and len(onnx_pads) == 4:
            ...
            if p_left == p_right and p_top == p_bottom:
                ...
                x = F.pad(
                    x, pad=convert_onnx_pads_to_torch(onnx_pads), value=float("-inf")
                )
                onnx_pads = 0
...
def Reshape_forward(
    ...
    shape = [
        shape[i].item() if hasattr(shape[i], "item") else shape[i]
...
def Resize_forward(
    ...
    size = (
        values[-1].cpu().tolist() if (len(values) == 4 and values[-1] != None) else None
...
def LSTM_forward(
    ...
    input_forget = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="input_forget", default=0
    )
