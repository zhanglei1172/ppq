...
OPERATION_FORWARD_TABLE = {
    platform: DEFAULT_BACKEND_TABLE.copy() for platform in TargetPlatform
}
...
class QuantOPRuntimeHook(RuntimeHook, metaclass=ABCMeta):
    ...
    def __init__(self, operation: QuantableOperation, **kwargs) -> None:
        if not isinstance(operation, QuantableOperation):
            raise TypeError(
                f"You are trying to bind a QuantRuntimeHook to a non-quantized operation {operation}."
            )
...
class BaseGraphExecutor(Callable, metaclass=ABCMeta):
    ...
    def tracing_operation_meta(
        self, inputs: Union[dict, list, torch.Tensor], output_names: List[str] = None
    ) -> None:
        raise NotImplementedError("Please implement this function first.")

    def __call__(
        self, inputs: Union[dict, torch.Tensor], output_names: List[str] = None
    ) -> List[torch.Tensor]:
