    ...
    def export_var(self, variable: Variable) -> onnx.TensorProto:
        if not variable.is_parameter:
            ...
            tensor_proto = helper.make_tensor(
                name=variable.name,
                data_type=variable.dtype.value,
                dims=variable.shape,
                vals=value,
            )
        return tensor_proto
                ...
                for config in configs:
                    ...
                    if config.can_export():
                        ...
                        min_tensor = numpy_helper.from_array(
                            convert_any_to_numpy(min_val).astype("float32"),
                            variable.name + MIN_SUFFIX,
                        )
                        min_info = helper.make_tensor_value_info(
                            variable.name + MIN_SUFFIX,
                            min_tensor.data_type,
                            min_tensor.dims,
                        )

                        max_tensor = numpy_helper.from_array(
                            convert_any_to_numpy(max_val).astype("float32"),
                            variable.name + MAX_SUFFIX,
                        )
                        max_info = helper.make_tensor_value_info(
                            variable.name + MAX_SUFFIX,
                            max_tensor.data_type,
                            max_tensor.dims,
                        )
