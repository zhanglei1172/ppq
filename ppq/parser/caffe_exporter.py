...
class CaffeExporter(GraphExporter):
    def export_quantization_config(self, config_path: str, graph: BaseGraph):
        ...
        for operation in graph.operations.values():
            ...
            for config, var in operation.config_with_variable:
                ...
                if (
                    ...
                    and var.name in var_quant_info_recorder
                ):
                    continue
        ...
        for var in var_quant_info_recorder:
            ...
            var_quant_info_recorder[var] = {
                ...
                "per_channel": config.policy.has_property(
                ...
                "tensor_min": convert_value(
                    config.scale * (config.quant_min - config.offset),
                    tensorwise,
                    DataType.FP32,
                ),
                "tensor_max": convert_value(
                    config.scale * (config.quant_max - config.offset),
                    tensorwise,
                    DataType.FP32,
                ),
    ...
    def prepare_model(self, graph: BaseGraph, input_shapes: List[List[int]]):
        ...
        if all([var.value is not None for var in graph.inputs.values()]):
            inputs = {
                var.name: convert_any_to_torch_tensor(var.value).to(device)
                for var in graph.inputs.values()
            }
        elif all([var.shape is not None for var in graph.inputs.values()]):
            inputs = {
                var.name: torch.randn(
                    *var.shape, dtype=DataType.to_torch(var.dtype), device=device
                )
                for var in graph.inputs.values()
            }
        else:
    ...
    def dump_to_file(
        self,
        caffe_model: ppl_caffe_pb2.NetParameter,
        caffe_proto: ppl_caffe_pb2.NetParameter,
        file_path: str,
    ):
    ...
    def export(
        ...
        **kwargs,
        ...
        self.dump_to_file(
            caffe_model=caffe_model, caffe_proto=caffe_proto, file_path=file_path
        )
            ...
            if layer_name not in graph.operations:
                ...
                if layer.type == "Slice":
                    ...
                    for bottom in layer.bottom:
                        ...
                        if (
                            var is not None
                            and isinstance(var, QuantableVariable)
                            and not var.is_parameter
                        ):
                            cfg = None
                            for dest_op, dest_cfg in zip(
                                var.dest_ops, var.dest_op_configs
                            ):
                                # dest_op.name is slice0_0
                                if (
                                    "".join(dest_op.name.split("_")[:-1])
                                    == original_name
                                ):
                                    cfg = dest_cfg
                                    break
                            assert cfg is not None
                            qt_min = convert_value(
                                cfg.scale * (cfg.quant_min - cfg.offset),
                                True,
                                DataType.FP32,
                            )
                            qt_max = convert_value(
                                cfg.scale * (cfg.quant_max - cfg.offset),
                                True,
                                DataType.FP32,
                            )
                            layer.quantize_param.add(
                                type="bottom", range_min=qt_min, range_max=qt_max
                            )
                    ...
                    for top in layer.top:
                        ...
                        if (
                            ...
                            layer.quantize_param.add(
                                type="top", range_min=qt_min, range_max=qt_max
                            )
                    continue
                else:
                    raise KeyError(
                        f"Can not find operation {layer_name} with current graph."
                    )
            ...
            if layer.type == "InnerProduct":
                for cfg, var in op.config_with_variable:
                    ...
                    if cfg.policy.has_property(QuantizationProperty.PER_CHANNEL):
                        ...
                        for _min, _max in zip(qt_min, qt_max):
                            p = (
                                layer.inner_product_param.perchannel_quantize_param.add()
                            )
                            p.type = "filter"
                            p.range_min = _min
                            p.range_max = _max
                    ...
                    for (
                        ...
                        if var.name == str(bottom):
                            ...
                            if var.name in graph.inputs:
                                range_min = convert_value(
                                    cfg.detail.get("range_min", -1), True, DataType.FP32
                                )
                ...
                if layer.type in {"Convolution", "Deconvolution"}:
                    for cfg, var in op.config_with_variable:
                        ...
                        if cfg.policy.has_property(QuantizationProperty.PER_CHANNEL):
                            ...
                            for _min, _max in zip(qt_min, qt_max):
                                p = (
                                    layer.convolution_param.perchannel_quantize_param.add()
                                )
                                p.type = "filter"
                                p.range_min = _min
                                p.range_max = _max
                ...
                for (
                    ...
                    if op.is_computing_op:
                        for _min, _max in zip(
                            cfg.detail["range_min"], cfg.detail["range_max"]
                        ):
                            layer.quantize_param.add(
                                type="topperchannel", range_min=_min, range_max=_max
                            )
