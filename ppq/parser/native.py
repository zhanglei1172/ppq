    ...
    def build(self, file_path: str, **kwargs) -> BaseGraph:
        ...
        with open(file_path, "rb") as file:
            ...
            if signature != "PPQ GRAPH DEFINITION":
                raise Exception(
                    "File format parsing error. Graph Signature has been damaged."
                )
        ...
        try:
            ...
            graph._graph_inputs = {
                name: graph.variables[name] for name in graph._graph_inputs
            }
            graph._graph_outputs = {
                name: graph.variables[name] for name in graph._graph_outputs
            }
