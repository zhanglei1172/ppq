...
class CaffeOpBuilder(object):
    ...
    def __init__(
        ...
        assert len(self.layer.bottom) == len(
            input_shape
        ), "Given input_shape and layer.bottom have different length"
    ...
    def infer_shape(self):
        ...
        input_value = [
            torch.from_numpy(tensor).to(self.device) for tensor in input_value
        ]
        input_value = dict([(x, y) for x, y in zip(input_names, input_value)])
        temp_graph = build_temp_graph(
            self.initializer, self.nodes, input_names, list(self.layer.top)
        )
...
class Convolution(CaffeOpBuilder):
    def get_attr(self):
        ...
        if conv.hole_h != 1 or conv.hole_w != 1:
            logger.warning(
                f"{self.layer.name} is dilated convolution, check if the real backend supports"
            )
...
class BatchNorm(CaffeOpBuilder):
    ...
    def get_attr(self):
        ...
        if not batchnorm.use_global_stats:
            logger.warning(
                "Now cannot convert BatchNorm layer with use_global_stats==False. Change it to True"
            )
...
class BN(CaffeOpBuilder):
    ...
    def get_attr(self):
        ...
        if batchnorm.moving_average:
            logger.warning(
                "Now cannot convert BatchNorm layer with moving_average==True. Change it to False"
            )
...
class Eltwise(CaffeOpBuilder):
    ...
    def trans(self):
        ...
        if len(self.layer.bottom) > len(self.layer.eltwise_param.coeff):
            coeff.extend(
                [1.0] * (len(self.layer.bottom) - len(self.layer.eltwise_param.coeff))
            )
        ...
        for i, name in enumerate(bottom_var[1:]):
            ...
            output_name = (
                node_name + "_out" if (i + 2 != len(bottom_var)) else self.layer.top[0]
            )
            op_type = self.op_type
            if coeff[i + 1] == -1.0:
                op_type = "Sub"
...
class Reshape(CaffeOpBuilder):
    def trans(self):
        ...
        self.initializer = {
            init_name: {"dims": [len(dim)], "value": np.array(dim, dtype="int64")}
        }
        self.nodes = [
            {
                "name": self.layer.name,
...
class InnerProduct(CaffeOpBuilder):
    ...
    def trans(self):
        ...
        assert (
            ip_param.num_output == self.layer.blobs[0].shape.dim[0]
        ), "Given num_output is not same with weight shape"
...
class Slice(CaffeOpBuilder):
    def trans(self):
        ...
        self.initializer[axis_name] = {
            "dims": [1],
            "value": np.array([axis], dtype="int64"),
        }

        slice_points = [0] + list(param.slice_point) + [self.input_shape[0][axis]]
        splits = [
            (slice_points[i], slice_points[i + 1]) for i in range(len(slice_points) - 1)
        ]
        for i, pairs in enumerate(splits):
            starts = self.layer.name + "_starts_" + str(i)
            ends = self.layer.name + "_ends_" + str(i)
            self.initializer[starts] = {
                "dims": [1],
                "value": np.array([pairs[0]], dtype="int64"),
            }
            self.initializer[ends] = {
                "dims": [1],
                "value": np.array([pairs[1]], dtype="int64"),
            }
            self.nodes.append(
                {
...
class ArgMax(CaffeOpBuilder):
    def get_attr(self):
        ...
        self.settings["axis"] = (
            argmax_param.axis if argmax_param.HasField("axis") else None
        )
        return self.settings
