...
class CaffeOpExporter(object):
    ...
    def parse(self) -> ppl_caffe_pb2.LayerParameter:
        self.set_attr()
        self.layer.bottom[:] = [
            var.name for var in self.op.inputs if not var.is_parameter
        ]
...
class Conv(CaffeOpExporter):
    def set_attr(self):
        ...
        dilations_h, dilations_w = refine_value(
            self.op.attributes.get("dilations", [1, 1])
        )
        ...
        if len(pads) == 2:
            ...
            logger.error(
                f"Unsupported pads attributes with the length of {len(pads)} in Caffe"
            )

        self.layer.convolution_param.num_output = self.op.parameters[0].value.shape[0]
        self.layer.convolution_param.group = refine_value(
            self.op.attributes.get("group", 1)
        )
...
class BatchNormalization(CaffeOpExporter):
    def set_attr(self):
        self.layer.bn_param.moving_average = bool(
            refine_value(self.op.attributes.get("training_mode", 0))
        )
        self.layer.bn_param.var_eps = refine_value(
            self.op.attributes.get("epsilon", 1e-05)
        )
        if self.layer.bn_param.moving_average:
            self.layer.bn_param.decay = 1 - refine_value(
                self.op.attributes.get("momentum", 0.9)
            )
...
class LeakyRelu(CaffeOpExporter):
    def set_attr(self):
        self.layer.relu_param.negative_slope = refine_value(
            self.op.attributes.get("alpha", 0.01)
        )
...
class ReduceL2(CaffeOpExporter):
    def set_attr(self):
        ...
        self.layer.reducel2_param.keepdims = refine_value(
            self.op.attributes.get("keepdims", 1)
        )
...
class Add(CaffeOpExporter):
    ...
    def parse(self):
        ...
        for i, var in enumerate(self.op.parameters):
            param_layer = ppl_caffe_pb2.LayerParameter(
                type="Parameter", name=self.op.name + "_param_" + str(i)
            )
...
class Reshape(CaffeOpExporter):
    def set_attr(self):
        if len(self.op.inputs) == 0:
            raise AttributeError(
                f"{self.op.name} has no inputs. Cannot convert to caffe op. "
...
class Gemm(CaffeOpExporter):
    def parse(self):
        ...
        if refine_value(self.op.attributes.get("transA", 0)) != 0:
            ...
            shape = A.shape
            if len(shape) == 2:
                transpose_layer = ppl_caffe_pb2.LayerParameter(
                    type="Transpose", name=self.op.name + "_transposed"
                )
        ...
        self.layer.inner_product_param.bias_term = (
            True if len(self.op.parameters) == 2 else False
        )
...
class Resize(CaffeOpExporter):
    def __init__(self, op):
        ...
        self.scales = (
            convert_any_to_numpy(op.inputs[2].value) if len(op.inputs) > 2 else []
        )
        self.sizes = (
            convert_any_to_numpy(op.inputs[-1].value) if len(op.inputs) == 4 else []
        )
        super().__init__(op)
    ...
    def set_attr(self):
        if self.op_type == "NNUpsample":
            ...
            valid_flag = (
                (self.scales[0] == 1.0)
                and (self.scales[1] == 1.0)
                and (self.scales[2] == self.scales[3])
            )
            if not valid_flag:
                raise AttributeError(
                    f"Cannot convert {self.op.name} to NNUpsample due to different scales"
                )
        ...
        if self.op_type == "Interp":
            ...
            trans_mode = refine_value(
                self.op.attributes.get("coordinate_transformation_mode", "half_pixel")
            )
            if trans_mode == "align_corners":
                ...
                raise AttributeError(
                    f"Cannot convert {self.op.name} in {trans_mode} mode to Interp."
                )
...
class ConvTranspose(CaffeOpExporter):
    def set_attr(self):
        ...
        self.layer.convolution_param.num_output = self.op.parameters[0].value.shape[
            1
        ] * self.op.attributes.get("group", 1)
        self.layer.convolution_param.group = refine_value(
            self.op.attributes.get("group", 1)
...
class Slice(CaffeOpExporter):
    def parse(self):
        ...
        starts, ends = convert_any_to_numpy(
            self.op.parameters[0].value
        ), convert_any_to_numpy(self.op.parameters[1].value)
        axes = (
            convert_any_to_numpy(self.op.parameters[2].value)
            if len(self.op.parameters) >= 3
            else [i for i in range(len(input_shape))]
        )
        if len(self.op.parameters) >= 4 and any(
            convert_any_to_numpy(self.op.parameters[3].value) != 1
        ):
        ...
        for i, (start_point, end_point, axis) in enumerate(
            list(zip(starts, ends, axes))
        ):
            current_layer = ppl_caffe_pb2.LayerParameter(
                type=self.op_type, name=self.op.name + "_" + str(i)
            )
...
class Scale(CaffeOpExporter):
    def set_attr(self):
        ...
        self.layer.scale_param.num_axes = refine_value(
            self.op.attributes.get("num_axes", 1)
        )
        self.layer.scale_param.bias_term = refine_value(
            self.op.attributes.get("bias_term", False)
        )
...
class ChannelShuffle(CaffeOpExporter):
    def set_attr(self):
        self.layer.channel_shuffle_param.group = refine_value(
            self.op.attributes.get("group", 1)
        )
...
class InstanceNormalization(CaffeOpExporter):
    def set_attr(self):
        self.layer.instance_norm_param.num_features = refine_value(
            self.op.attributes.get("num_features")
        )
        self.layer.instance_norm_param.eps = refine_value(
            self.op.attributes.get("eps", 1e-5)
        )
        self.layer.instance_norm_param.affine = refine_value(
            self.op.attributes.get("affine", False)
        )
...
class Parameter(CaffeOpExporter):
    def set_attr(self):
        ...
        self.layer.parameter_param.batch = refine_value(
            self.op.attributes.get("batch", 1)
        )
        self.layer.parameter_param.channel = refine_value(
            self.op.attributes.get("channel", -1)
        )
        self.layer.parameter_param.height = refine_value(
            self.op.attributes.get("height", -1)
        )
        self.layer.parameter_param.width = refine_value(
            self.op.attributes.get("width", -1)
        )
...
class Interp(CaffeOpExporter):
    def set_attr(self):
        if refine_value(self.op.attributes.get("shrink_factor")) != 1:
            self.layer.interp_param.shrink_factor = refine_value(
                self.op.attributes.get("shrink_factor")
            )
        if refine_value(self.op.attributes.get("zoom_factor")) != 1:
            self.layer.interp_param.zoom_factor = refine_value(
                self.op.attributes.get("zoom_factor")
            )
        if refine_value(self.op.attributes.get("width")) and refine_value(
            self.op.attributes.get("height")
        ):
            self.layer.interp_param.height = refine_value(
                self.op.attributes.get("height")
            )
            self.layer.interp_param.width = refine_value(
                self.op.attributes.get("width")
            )
        self.layer.interp_param.pad_beg = refine_value(
            self.op.attributes.get("pad_beg")
        )
        self.layer.interp_param.pad_end = refine_value(
            self.op.attributes.get("pad_end")
        )
        self.layer.interp_param.align_corners = refine_value(
            self.op.attributes.get("align_corners")
        )
...
class Flatten(CaffeOpExporter):
    def set_attr(self):
        ...
        self.layer.flatten_param.end_axis = refine_value(
            self.op.attributes.get("end_axis", -1)
        )
...
class SpaceToDepth(CaffeOpExporter):
    def set_attr(self):
        self.layer.subpixel_down_param.downsample = refine_value(
            self.op.attributes.get("blocksize", 1)
        )
...
class DepthToSpace(CaffeOpExporter):
    def set_attr(self):
        self.layer.subpixel_up_param.upsample = refine_value(
            self.op.attributes.get("blocksize", 1)
        )
...
class CaffeArgMax(CaffeOpExporter):
    def set_attr(self):
        self.layer.argmax_param.out_max_val = refine_value(
            self.op.attributes.get("out_max_val")
        )
