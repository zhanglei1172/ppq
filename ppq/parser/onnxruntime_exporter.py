...
class ONNXRUNTIMExporter(OnnxExporter):
    ...
    def insert_quantize_node(
        ...
        if config.policy.has_property(QuantizationProperty.LINEAR):
            ...
            offset = convert_any_to_torch_tensor(
                config.offset.clone(), dtype=torch.float32
            )
    ...
    def remove_duplicated_quant_op(self, graph: BaseGraph) -> BaseGraph:
        ...
        for qt_op, dq_op in interested_pairs:
            ...
            scale_diff = torch.max(
                torch.abs(dq_op.inputs[1].value - qt_op.inputs[1].value)
            ).item()
            zeropoint_diff = torch.max(
                torch.abs(dq_op.inputs[2].value - qt_op.inputs[2].value)
            ).item()
            if (
                scale_diff < 1e-5 and zeropoint_diff < 0.5
            ):  # zero point 是整数，所以只要误差小于1就行了。
                # mark quant operation and its following operation(suppose to be another dequantization op)
                mark_to_remove.add(qt_op)
                assert (
                    len(graph.get_downstream_operations(qt_op)) == 1
                ), "Oops, that should never happen."
