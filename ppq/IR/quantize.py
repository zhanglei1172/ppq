...
class QuantableOperation(Operation):
    ...
    def store_parameter_value(self):
        for var, _ in zip(
            ...
            if var.is_parameter:
                ...
                var.stored_value = convert_any_to_torch_tensor(
    ...
    def dequantize(self, parameter_only: bool = False, expire_device: str = "cpu"):
        ...
        for var, quant_config in zip(
            ...
            ), f"Unexpected error with variable {var.name}."
            if var.is_parameter:
                # convert var.value to torch.Tensor
                # notice here we set device = None, this conversion will not change var.value.device anyway.
                # so that we can use var.value.device as a deploy device for stored_value
                stored_value = convert_any_to_torch_tensor(
                ...
                var.value = convert_any_to_torch_tensor(
                    var.stored_value,
                    device=var.value.device if var.value is not None else None,
                )
...
class QuantableVariable(Variable):
    def __init__(self, convert_from: Variable) -> None:
        ...
        if convert_from.value is not None:
            self._fp32_value = convert_any_to_torch_tensor(
                convert_from.value, device="cpu"
            )
...
class DeviceSwitchOP(Operation):
    ...
    def __init__(
        self, name: str, inputs: List[Variable] = None, outputs: List[Variable] = None
    ) -> None:
        super().__init__(
            attributes={},
            name=name,
...
class QuantableGraph(GraphCommandProcessor):
    ...
    def quantize_operation(
        ...
        if operation_name not in self.graph.operations:
            raise KeyError(
                f"Operation {operation_name} is not in your graph, Please check your input."
            )
