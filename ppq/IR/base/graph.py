...
class Variable(Serializable):
    ...
    def meta(self) -> TensorMeta:
        raise Exception(
            "PPQ Variable.meta has been removed since 0.6.6, use Variable.shape, Variable.dtype instead."
        )
    ...
    def __getstate__(self) -> dict:
        ...
        state["_source_op"] = (
            self.source_op.name if self.source_op is not None else None
        )
        return state
...
class Operation(OperationBase, Serializable):
    def __init__(
        ...
        OperationBase.__init__(
            self, name, op_type, attributes, platform=platform, opset=opset
        )
...
class BaseGraph(Serializable):
    ...
    def __init__(
        self, name: str, built_from: NetworkFramework = NetworkFramework.NATIVE
    ) -> None:
        super().__init__()
        self._operations = {}
        self._variables = {}
    ...
    def append_operation(self, operation: Operation):
        if not isinstance(operation, Operation):
            raise TypeError(
                f"You can only insert operations via this function, however {type(operation)} was given."
            )
        if not all(
            [var.name in self.variables for var in operation.inputs + operation.outputs]
        ):
            raise KeyError(
        ...
        if operation.name in self.operations:
            raise KeyError(
                f"Duplicated Operation({operation}) was found, rename your Operation before inserting."
            )
    ...
    def append_variable(self, var: Variable):
        if not isinstance(var, Variable):
            raise TypeError(
                f"You can only insert variable via this function, however {type(var)} was given."
            )
        ...
        if var.name in self.variables:
            raise KeyError(
                f"Duplicated Variable({var}) was found, rename your Variable before inserting."
            )
    ...
    def get_downstream_operations(self, operation: Operation) -> List[Operation]:
        if not isinstance(operation, Operation):
            raise TypeError(
                f"Expect an operation instance, however {type(operation)} is given."
            )
    ...
    def insert_op_between(
        self, inserting_op: Operation, up_op: Operation, down_op: Operation
    ):
    ...
    def insert_op_between_var_and_op(
        self, inserting_op: Operation, up_var: Variable, down_op: Operation
    ):
        ...
        if up_var.name not in self.variables:
            raise KeyError(
                f"Can not inserting operation behind {up_var.name}, variable not found."
            )
        if down_op.name not in self.operations:
            raise KeyError(
                f"Can not inserting operation behind {down_op.name}, operation not found."
            )
        ...
        assert len(variables) == 1, (
            f"Can not insert operation between {var.name} and {down_op.name},"
        ...
        **kwargs,
        ...
        if dest_ops is not None:
            ...
            for op in dest_ops:
                if not isinstance(op, Operation):
                    raise TypeError(
                        f"Parameter dest ops should be a list of Operation, however {type(op)} was given."
                    )
        ...
        if source_op is not None:
            if not isinstance(source_op, Operation):
                raise TypeError(
                    f"Parameter dest ops should be an Operation, however {type(source_op)} was given."
                )
    ...
    def mark_variable_as_graph_input(self, var: Variable):
        ...
        if var_name in self.outputs:
            raise KeyError(
                f"Can not mark variable {var_name} as graph input, cause it is graph output."
            )
