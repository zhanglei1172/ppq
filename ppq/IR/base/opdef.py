...
class Opset:
    def __init__(
        self, domain: str = DEFAULT_OPSET_DOMAIN, version: int = DEFAULT_OPSET_VERSION
    ) -> None:
...
class VLink:
    def __init__(self, in_idx: int, out_idx: int) -> None:
        if not isinstance(in_idx, int):
            raise TypeError(
                f"Can not create vlink with input_idx {in_idx}, "
                "only int value is acceptable here."
            )
        if not isinstance(out_idx, int):
            raise TypeError(
                f"Can not create vlink with output_idx {out_idx}, "
                "only int value is acceptable here."
            )
        self.in_idx = in_idx
...
class OpSocket:
    def __init__(
        ...
        if out_plat is None:
            self.out_plat = [
                TargetPlatform.UNSPECIFIED for _ in range(op.num_of_output)
            ]
...
def Resize_Socket(op: OperationBase) -> OpSocket:
    ...
    CHECK_OPSET(
        op=op, min_version_supported=10, max_version_supported=13, strict_check=True
    )
    in_plat = [
