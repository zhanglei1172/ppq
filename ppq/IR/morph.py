...
class GraphFormatter(GraphCommandProcessor):
    ...
    def process(self, command: GraphCommand) -> Any:
        ...
        if command.command_type == GraphCommandType.TRUNCATE_ON_VAR:
            ...
            ), f"Use TruncateGraphCommand here."
    ...
    def format_clip(self) -> None:
        ...
        for op in interested_ops:
            ...
            min_var = Variable(
                name=op.name + "_min", value=min, is_parameter=True, dest_ops=[op]
            )
            max_var = Variable(
                name=op.name + "_max", value=max, is_parameter=True, dest_ops=[op]
            )
            ...
            if len(operation.inputs) == 2:
                ...
                if index_op.type == "Constant":
                    ...
                    operation.attributes[
                        "gather_index"
                    ] = convert_any_to_python_primary_type(index)
    ...
    def format_cast(self) -> None:
        ...
        for operation in interested_ops:
            ...
            operation.attributes["to"] = DataType.convert_from_numpy(
                operation.attributes["to"]
            )
    ...
    def truncate_on_var(self, var: Variable, mark_as_output: bool):
        ...
        if not isinstance(var, Variable):
            raise TypeError(
                f"Except variable instance here, however {type(var)} was given."
            )
    ...
    def __delete_constant_input(self, op: Operation, input_idx: int):
        ...
        if input_var.source_op.type != "Constant":
            raise ValueError(
                f"Trying to delete an non-const input, "
...
class GraphMerger(GraphCommandProcessor):
    ...
    def fuse_gemm(self):
        ...
        def _is_replaceable(op: Operation) -> bool:
            if (
                op.inputs[0].is_parameter == False
                and op.inputs[1].is_parameter == False
            ):
                return False
            else:
                return True
                ...
                if len(graph.get_downstream_operations(op)) == 1:
                    ...
                    if down.type == "Add":
                        ...
                        if op.num_of_input == 3:  # already has a bias
                            ...
                            graph.remove_operation(
                                removing_op=down, keep_coherence=True
                            )
    ...
    def decompose_gemm(self):
        ...
        for op in interested_ops:
            ...
            if op.num_of_input == 3:
                ...
                graph.create_link_with_op(
                    variable=graph.create_variable(
                        value=bias_var.value * op.attributes.get("beta", 1),
                        is_parameter=True,
                    ),
                    A=None,
                    B=bias_add,
            ...
            if op.attributes.get("transA", 0) == 1:
                raise ValueError(
                    f"Can not process with operation {op.name}, transA=1 is not allowed."
                )
