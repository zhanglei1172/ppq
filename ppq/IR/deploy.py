...
import torch
from ppq.core import TargetPlatform, convert_any_to_numpy, convert_any_to_torch_tensor
    ...
    def retrieve(self):
        for _, operator in self._graph.operations.items():
            ...
            if operator.type == "Constant":
                operator.attributes["value"] = convert_any_to_numpy(
    ...
    def deploy(self, device: str):
        ...
        for _, variable in self._graph.variables.items():
            ...
            if (
                ...
                and platforms[0] == TargetPlatform.SOI
            ):
            ...
            if platform == TargetPlatform.SOI:
                variable.value = convert_any_to_torch_tensor(
                    variable.value, accept_none=True
                ).to("cpu")
            else:
                variable.value = convert_any_to_torch_tensor(
                    variable.value, accept_none=True
