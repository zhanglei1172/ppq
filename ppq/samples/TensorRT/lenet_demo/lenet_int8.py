...
def generateLenetEngine(weight_map, json_file, engine_path):
    ...
    data = network.add_input(
        INPUT_BLOB_NAME, trt.float32, (1, CHANNEL, INPUT_H, INPUT_W)
    )
    assert data
    data.name = "input.1"
    ...
    fc3 = network.add_fully_connected(
        input=relu4.get_output(0),
...
def setDynamicRange(network, json_file):
    ...
    for i in range(network.num_inputs):
        ...
        if act_quant.__contains__(input_tensor.name):
            print(
                "\033[1;32mWrite quantization parameters:%s\033[0m" % input_tensor.name
            )
    ...
    for i in range(network.num_layers):
        ...
        for output_index in range(layer.num_outputs):
            ...
            if act_quant.__contains__(tensor.name):
                ...
                print(
                    "\033[1;31mNo quantization parameters are written: %s\033[0m"
                    % tensor.name
                )
