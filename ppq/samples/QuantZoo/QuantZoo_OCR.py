...
with ENABLE_CUDA_KERNEL():
    for config in CONFIGS:
        ...
        graph = load_onnx_graph(
            onnx_import_file=os.path.join(MODEL_DIR, model + ".onnx")
        )
        ...
        executor.tracing_operation_meta(
            torch.zeros(size=[BATCHSIZE, 3, 32, 100]).cuda()
        )
        ...
        acc = evaluate_ppq_module_with_ic15(
            executor=executor,
            character_dict_path=os.path.join(MODEL_DIR, dictionary),
            dataloader=test_loader,
            collate_fn=lambda x: convert_any_to_torch_tensor(x).cuda(),
        )
