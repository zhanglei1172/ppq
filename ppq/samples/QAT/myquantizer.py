...
)  # check ppq.samples.imagenet.Utilities
...
class MyQuantizer(BaseQuantizer):
    def init_quantize_config(self, operation: Operation) -> OperationQuantizationConfig:
        ...
        if operation.name == "MyLayer" and operation.type in {
            "Conv",
            "ConvTranspose",
            "Gemm",
            "MatMul",
        ...
        if operation.type not in self.quant_operation_types:
            print(
                f"Operation {operation.name}({operation.type}) is not supported with current Quantizer."
            )
...
dispatching_table = PFL.Dispatcher(graph=graph).dispatch(
