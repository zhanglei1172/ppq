...
def load_graph(
    ...
    if from_framework == NetworkFramework.CAFFE:
        ...
        graph = parser.build(
            prototxt_path=file_path, caffemodel_path=kwargs["caffemodel_path"]
        )
    else:
...
def load_torch_model(
    ...
    onnx_file_name: str = "MyModel.onnx",
    sample: Union[dict, list, torch.Tensor] = None,
    device="cuda",
    verbose: bool = False,
    opset: int = 11,
    ...
    if opset not in {11, 13}:
        ppq_warning(
            f"The Opset of Exporting Model should be 11 or 13, however {opset} was given."
        )
    ...
    _training = (
        torch.onnx.TrainingMode.TRAINING if training else torch.onnx.TrainingMode.EVAL
    )
    torch.onnx.export(
        model=model,
        args=sample,
...
def export_ppq_graph(
    ...
    **kwargs,
    ...
    for save_path in [graph_save_to, config_save_to]:
        ...
        if os.path.exists(save_path):
            if os.path.isfile(save_path):
                ppq_warning(
                    f"File {save_path} is already existed, Exporter will overwrite it."
                )
            if os.path.isdir(save_path):
                raise FileExistsError(
                    f"File {save_path} is already existed, and it is a directory, "
    ...
    if copy_graph:
        graph = graph.copy()
    exporter.export(
        file_path=graph_save_to, config_path=config_save_to, graph=graph, **kwargs
    )
...
class UnbelievableUserFriendlyQuantizationSetting:
    ...
    def to_json(self, file_path: str) -> str:
        ...
        with open(file_path, "w", encoding="utf-8") as file:
            json.dump(
                obj=dump_dict, fp=file, sort_keys=True, indent=4, ensure_ascii=False
            )
...
def quantize(
    ...
    if model_type == NetworkFramework.ONNX:
        if not os.path.exists(os.path.join(working_directory, "model.onnx")):
            raise FileNotFoundError(
                f'无法找到你的模型: {os.path.join(working_directory, "model.onnx")},'
    ...
    if model_type == NetworkFramework.CAFFE:
        if not os.path.exists(os.path.join(working_directory, "model.caffemodel")):
            raise FileNotFoundError(
                f'无法找到你的模型: {os.path.join(working_directory, "model.caffemodel")},'
...
def export(
    working_directory: str, quantized: BaseGraph, platform: TargetPlatform, **kwargs
):
