...
def load_calibration_dataset(
    ...
    for file in os.listdir(os.path.join(directory, "data")):
        ...
        if file.endswith(".npy"):
            ...
            sample = np.fromfile(
                os.path.join(directory, "data", file), dtype=np.float32
            )
            assert isinstance(
                sample, np.ndarray
            ), f"数据应当是 numpy.ndarray，然而你输入了 {type(sample)}"
        ...
        if sample.shape[1] not in {1, 3, 4}:
            ppq_warning(
                f"Quantized data is not a regular channel, which channel is {sample.shape[1]}."
            )

        assert (
            sample.shape[0] == 1 or batchsize == 1
...
def load_from_file(
    ...
    if not os.path.exists(filename) or os.path.isdir(filename):
        raise FileNotFoundError(
            f"Can not load data from file: {filename}. "
...
def dump_to_file(
    filename: str, data: Union[torch.Tensor, np.ndarray], format: str = ".npy"
) -> None:
    if os.path.isdir(filename):
        raise FileExistsError(
            f"Can not dump data to file: {filename}", " Cause it is a directory."
        )
    ...
    for var in graph.variables.values():
        if isinstance(var, QuantableVariable):
            if (
                ...
                and var.source_op_config.state == QuantizationStates.ACTIVATED
            ):
                graph.outputs[var.name] = var  # 直接标记为网络输出

    executor = TorchExecutor(graph, device=executing_device)
    for batch_idx, batch in tqdm(
        enumerate(dataloader), total=len(dataloader), desc="Dumping Results ..."
    ):
        ...
        for name, output in zip(graph.outputs, outputs):
            ...
            if output.numel() > 10000 and sample:
                output = tensor_random_fetch(
                    tensor=output, seed=10086, num_of_fetches=4096  # 保证随机种子一致才能比对结果
                )
...
def split_result_to_directory(raw_dir: str, to_dir: str):
    ...
    for sample_name, file_name in tqdm(
        data_files, total=len(data_files), desc="正复制文件中 ..."
    ):
        file_path = os.path.join(raw_dir, file_name)
        processed = os.path.join(
            to_dir, sample_name, file_name[file_name.index("_") + 1 :]
        )
