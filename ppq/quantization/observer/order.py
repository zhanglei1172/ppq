...
class TorchIsotoneObserver(BaseTensorObserver):
    def __init__(self, watch_on: Variable, quant_cfg: TensorQuantizationConfig):
        ...
        if OBSERVER_ISOTONE_OBSERVER_AXIS not in quant_cfg.detail:
            ppq_warning(
                "Initializing Torch Isotone Observer with implicit axis is not recommended."
            )
            self.axis = -1
        else:
    ...
    def observe(self, value: torch.Tensor):
        ...
        if self._quant_cfg.state == QuantizationStates.INITIAL:
            if self._quant_cfg.policy.has_property(QuantizationProperty.PER_TENSOR):
                ...
                value, _ = torch.topk(
                    value, k=2, dim=self.axis, largest=True, sorted=True
                )
                if value.ndim <= 1:
                ...
                raise TypeError(
                    "Isotone Observer is not designed for channelwise quantization."
                )
            else:
                raise TypeError(
                    "Isotone Observer only work with per-tensor or per-channel quantize policy."
                )
    ...
    def render_quantization_config(self):
        ...
        if len(s_candidates) <= 0:
            ...
            scale, offset = minmax_to_scale_offset(
                min_val=0, max_val=l1, config=self._quant_cfg
            )
