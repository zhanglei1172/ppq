...
def minmax_to_scale_offset(
    ...
    if config.policy.has_property(QuantizationProperty.ASYMMETRICAL):
        ...
        raise TypeError(
            "Tensor Min Max Observer Excepts either ASYMMETRICAL or SYMMETRICAL quantization config."
        )
    ...
    def observe(self, value: torch.Tensor):
        ...
        if self._quant_cfg.state == QuantizationStates.INITIAL:
            if self._quant_cfg.policy.has_property(QuantizationProperty.PER_TENSOR):
                ...
                channelwise_view = value.transpose(dim0=0, dim1=channel_axis).unsqueeze(
                    -1
                )
                ...
                raise TypeError(
                    "Min-max Observer only work with per-tensor or per-channel quantize policy."
                )
    ...
    def render_quantization_config(self):
        ...
        if self._quant_cfg.policy.has_property(QuantizationProperty.PER_TENSOR):
            scale, offset = minmax_to_scale_offset(
                min_val=torch.min(torch.cat(self._min_val_collector, dim=0))
                .cpu()
                .item(),
                max_val=torch.max(torch.cat(self._max_val_collector, dim=0))
                .cpu()
                .item(),
        ...
        if self._quant_cfg.state == QuantizationStates.INITIAL:
            if self._quant_cfg.policy.has_property(QuantizationProperty.PER_TENSOR):
                if not PPQ_CONFIG.USING_CUDA_KERNEL or (not value.is_cuda):
                    ...
                    _min = torch.kthvalue(value.flatten(), k=min_idx, dim=0)[0].view(
                        1, -1
                    )
                    _max = torch.kthvalue(value.flatten(), k=max_idx, dim=0)[0].view(
                        1, -1
                    )
                ...
                raise PermissionError(
                    "Percentile observer can not deal with per channel quantization."
                )
                channel_axis = self._quant_cfg.channel_axis
    ...
    def hist_to_scale_offset(
        ...
        if config.policy.has_property(
            ...
            loss = self.compute_mse_loss(
                histogram=histogram, start=0, step=step, end=num_of_quant_levels * step
            )
