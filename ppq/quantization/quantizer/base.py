...
class BaseQuantizer(metaclass=ABCMeta):
    ...
    def quantize_operation(
        ...
        if op_name not in self._graph.operations:
            raise KeyError(
                f"Can not find op {op_name} in your graph, chech operation name again."
            )
        converting_operation = self._graph.operations[op_name]
        if isinstance(converting_operation, QuantableOperation):
            ppq_warning(
                f"Operation {op_name} has been quantized, can not to quantize it twice."
            )
    ...
    def build_quant_pipeline(
        ...
        if setting.quantize_parameter:
            ...
            list_of_passes.append(
                ParameterQuantizePass(method=param_setting.calib_algorithm)
            )

        if setting.quantize_activation:
            act_setting = setting.quantize_activation_setting
            list_of_passes.append(
                RuntimeCalibrationPass(method=act_setting.calib_algorithm)
            )
        ...
        if setting.extension:
            list_of_passes.append(
                ExtensionPass(setting.extension_setting.my_first_parameter)
            )
