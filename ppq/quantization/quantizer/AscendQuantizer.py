...
def ASSERT_CONV_AND_DECONV(op: Operation):
    ...
    if op.type == "ConvTranspose":
        filter = op.inputs[1].value.shape
        dilation = GET_ATTRIBUTE_FROM_OPERATION(
            op=op, attribute="dilations", compulsive=True, default=1
        )
        groups = GET_ATTRIBUTE_FROM_OPERATION(
            op=op, attribute="group", compulsive=True, default=1
        )
...
def ASSERT_GEMM(op: Operation):
    axis = GET_ATTRIBUTE_FROM_OPERATION(
        op=op, attribute="axis", compulsive=True, default=1
    )
    assert (
        axis == 1
...
class AscendQuantizer(BaseQuantizer):
    ...
    def init_quantize_config(self, operation: Operation) -> OperationQuantizationConfig:
        ...
        if operation.type in {"Conv", "Gemm", "ConvTranspose"}:
            ...
            ), "Seems you got a Computing layer with no parameters."
