...
class RuntimeCalibrationPass(QuantizationOptimizationPass):
    ...
    * override(bool)
    ...
    def __init__(
        self, method: str = None, override: bool = False, calib_steps: int = 32
    ) -> None:
    ...
    def optimize(
        ...
        **kwargs,
        ...
        assert calib_steps <= 512, (
            "Calibration steps is too large, ppq can quantize your network within 8-512 "
        ...
        for op_name, operation in graph.operations.items():
            ...
            observer = OperationObserver(
                operation=executor._graph.operations[op_name], monitor_parameter=False
            )
    ...
    def optimize(
        ...
        **kwargs,
        ...
        for operation in tqdm(
            graph.topological_sort(), desc="Collecting Observer For Computing Ops"
        ):
            if (
                not isinstance(operation, QuantableOperation)
                or not operation.is_computing_op
            ):
                continue
            output_cfg = operation.config.output_quantization_config[0]
            master_cfg, master_operation, master_var = (
                output_cfg,
                operation,
                operation.outputs[0],
            )
            ...
            if operation.inputs[0].name in graph.inputs:
                ...
                observe_table.update(
                    {
                        input_cfg: TensorObserverFactroy.build_observer(
                            operation.inputs[0], sym_input_cfg
                        )
                    }
                )
            ...
            observe_table.update(
                {
                    master_cfg: TensorObserverFactroy.build_observer(
                        master_var, master_cfg_per_channel
                    )
                }
            )
            assert (
                master_operation.name not in hooks
            ), "register an operation in calibration hooks twice"
            hooks[master_operation.name] = CalibrationHook(
                master_operation, observe_table
            )

        self.calibrate(
            desc="ReCalibration For Computing Ops",
            dataloader=dataloader,
            executor=executor,
            hooks=hooks,
        )
        ...
        for hook in hooks.values():
            ...
            for observer in hook._observer_table.values():
                ...
                if observer._quant_cfg.policy.has_property(
                    QuantizationProperty.PER_CHANNEL
                ):
                    min_vals = (
                        torch.min(
                            torch.cat(observer._min_val_collector, dim=-1),
                            dim=-1,
                            keepdim=False,
                        )[0]
                        .cpu()
                    ...
                    max_vals = (
                        torch.max(
                            torch.cat(observer._max_val_collector, dim=-1),
                            dim=-1,
                            keepdim=False,
                        )[0]
                        .cpu()
                ...
                elif observer._quant_cfg.policy.has_property(
                    QuantizationProperty.PER_TENSOR
                ):
                    min_val = (
                        torch.min(torch.cat(observer._min_val_collector, dim=0))
                        .cpu()
                        .item(),
                    )
                    max_val = (
                        torch.max(torch.cat(observer._max_val_collector, dim=0))
                        .cpu()
                        .item(),
                    )
