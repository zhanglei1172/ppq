        ...
        if self.fuse_activation:
            patterns = processor.pattern_matching(
                patterns=[
                    ...
                    lambda x: x.type in self.activation_types,
                ],
                edges=[[0, 1]],
                exclusive=True,
            )
            ...
            for computing_op, act_op in patterns:
                ...
                if (
                    ...
                    act_op.config.input_quantization_config[
                        0
                    ].dominated_by = act_op.config.output_quantization_config[0]
            ...
            if "Swish" in self.activation_types:
                ...
                for pattern in patterns:
                    ...
                    computing.config.output_quantization_config[
                        0
                    ].dominated_by = master_config
                    sigmoid.config.input_quantization_config[
                        0
                    ].dominated_by = master_config
                    sigmoid.config.output_quantization_config[
                        0
                    ].dominated_by = master_config
            ...
            if "Mish" in self.activation_types:
                ...
                for pattern in patterns:
                    ...
                    tanh.config.input_quantization_config[
                        0
                    ].dominated_by = master_config
                    tanh.config.output_quantization_config[
                        0
                    ].dominated_by = master_config
                    softplus.config.input_quantization_config[
                        0
                    ].dominated_by = master_config
                    softplus.config.output_quantization_config[
                        0
                    ].dominated_by = master_config
                    mul.config.input_quantization_config[0].dominated_by = master_config
                    mul.config.input_quantization_config[1].dominated_by = master_config
        ...
        **kwargs,
        ...
        for variable in graph.variables.values():
            if isinstance(variable, QuantableVariable):
                if variable.source_op is None or not isinstance(
                    variable.source_op, QuantableOperation
                ):
