        ...
        for op in graph.operations.values():
            if op.type == "Gemm":
                ...
                if op.attributes.get("alpha", 1.0) != 1.0:
                    op.parameters[0].value = op.parameters[0].value * op.attributes.get(
                        "alpha", 1.0
                    )
                    op.attributes["alpha"] = 1.0
                if op.attributes.get("beta", 1.0) != 1.0:
                    if op.num_of_input > 2:
                        op.parameters[1].value = op.parameters[
                            1
                        ].value * op.attributes.get("beta", 1.0)
...
class HorizontalLayerSplitPass(QuantizationOptimizationPass):
    ...
    def h_split(self, op: Operation) -> Tuple[torch.Tensor, torch.Tensor, int]:
        ...
        if self.verbose:
            print("")
            print(
                f"# Layer {op.name} has been splited, "
        ...
        with torch.no_grad():
            for name in self.interested_layers:
                ...
                if op1.type not in {"Gemm", "MatMul", "Conv", "ConvTranspose"}:
                    raise TypeError(
                        f"Operation {op1.name} can not be splited, op type is invalid({op1.type})"
                    )
                if not op1.inputs[1].is_parameter:
                    raise ValueError(
                        f"Operation {op1.name} can not be splited, input 1 is not parameter."
                    )
                ...
                if processed_values > 0:
                    ...
                    graph.create_link_with_op(
                        variable=op1.inputs[0], A=input_var.source_op, B=op2
                    )
                    ...
                    adder = graph.create_operation(
                        op_type="Add", platform=op1.platform, outputs=[output_var]
                    )
