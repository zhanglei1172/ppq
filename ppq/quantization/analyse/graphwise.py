...
class DetailedRecorder(RuntimeHook):
    ...
    def pre_forward_hook(self, inputs: List[torch.Tensor], **kwargs) -> list:
        for idx, input in enumerate(inputs):
            self.i_storage[idx].append(
                tensor_random_fetch(input, seed=10086, num_of_fetches=self.fetchs).to(
                    "cpu"
                )
            )
        return super().pre_forward_hook(inputs, **kwargs)
    ...
    def post_forward_hook(self, outputs: List[torch.Tensor], **kwargs) -> list:
        for idx, output in enumerate(outputs):
            self.o_storage[idx].append(
                tensor_random_fetch(output, seed=10086, num_of_fetches=self.fetchs).to(
                    "cpu"
                )
            )
        return super().post_forward_hook(outputs, **kwargs)
...
def graphwise_error_analyse_v2(
    graph: BaseGraph,
    running_device: str,
    dataloader: Iterator,
    ...
    interested_op = (
        ...
        if (interested_op is None or len(interested_op) == 0)
        else interested_op
    )
    if len(interested_op) == 0:
        print("Oops. you got nothing to analyse.")
