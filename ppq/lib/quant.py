...
def Pipeline(
    optims: List[QuantizationOptimizationPass],
) -> QuantizationOptimizationPipeline:
    """
...
def Observer(
    quant_config: TensorQuantizationConfig, variable: Variable = None
) -> BaseTensorObserver:
    """
    ...
    def render(self):
        if self._batch_observed == 0:
            raise PermissionError(
                "You have not provide any data to this QuantStub, "
...
class ParameterQuant(TensorQuant):
    def __init__(
        self, quant_config: TensorQuantizationConfig, parameter: torch.Tensor
    ) -> None:
        if not isinstance(parameter, torch.Tensor):
            raise TypeError(
                f"Expect a torch.Tensor here. However {type(parameter)} was given."
            )
...
def LinearQuantizationConfig(
    ...
    sym = (
        ...
        if symmetrical
        else QuantizationProperty.ASYMMETRICAL
    )
    ...
    chn = (
        ...
        else QuantizationProperty.PER_CHANNEL
    )
...
def Dispatcher(graph: BaseGraph, method: str = "conservative") -> GraphDispatcher:
    ...
    if method not in DISPATCHER_TABLE:
        raise KeyError(
            f"Can not find a dispatcher named {method}, check your input again."
        )
    return DISPATCHER_TABLE[method](graph)


def OperationForwardFunction(
    optype: str, platform: TargetPlatform = TargetPlatform.FP32
) -> Callable:
    """
    ...
    if optype not in OPERATION_FORWARD_TABLE[platform]:
        raise KeyError(
            f"Can not find a forward function related to optype {optype}({platform.name}),"
            " Register it first."
        )
...
def Exporter(platform: TargetPlatform) -> GraphExporter:
    ...
    if platform not in __EXPORTERS__:
        raise KeyError(
            f"Platfrom {platform.name} has no related exporter, register a exporter for it first."
        )
...
def Parser(framework: NetworkFramework) -> GraphExporter:
    ...
    if not isinstance(framework, NetworkFramework):
        raise TypeError(
            "Parameter framework has invalid type, Check your parameter again."
        )
