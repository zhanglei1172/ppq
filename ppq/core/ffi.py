...
class ComplieHelper(metaclass=SingletonMeta):
    ...
    def complie(self):
        ...
        lock_file = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "csrc/build/lock"
        )
        ...
        self.__CUDA_EXTENTION__ = load(
            ...
            sources=[
                os.path.join(
                    os.path.dirname(os.path.dirname(__file__)), "csrc/export.cc"
                ),
                os.path.join(
                    os.path.dirname(os.path.dirname(__file__)), "csrc/cuda/linear.cu"
                ),
                os.path.join(
                    os.path.dirname(os.path.dirname(__file__)), "csrc/cuda/sort.cu"
                ),
                os.path.join(
                    os.path.dirname(os.path.dirname(__file__)), "csrc/cuda/train.cu"
                ),
                os.path.join(
                    os.path.dirname(os.path.dirname(__file__)), "csrc/cuda/floating.cu"
                ),
                os.path.join(
                    os.path.dirname(os.path.dirname(__file__)), "csrc/cpu/hist_mse.cc"
                ),
            ],
            build_directory=os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "csrc/build/"
            ),
            with_cuda=True,
            extra_cflags=["-O3"],
        )
...
class CUDA:
    ...
    def Histogram_T(
        ...
        # if scale < 1e-7: raise ValueError('scale is too small.')
        CUDA_COMPLIER.CUDA_EXTENSION.Histogram_T(
            tensor, scale, clip_outliers, histogram
        )
        return histogram
    ...
    def Histogram_Asymmetric_T(
        ...
        # if scale < 1e-7: raise ValueError('scale is too small.')
        CUDA_COMPLIER.CUDA_EXTENSION.Histogram_Asymmetric_T(
    ...
    def Histogram_C(
        ...
        # if scale < 1e-7: raise ValueError('scale is too small.')
        CUDA_COMPLIER.CUDA_EXTENSION.Histogram_C(
    ...
    def compute_mse_loss(histogram: list, start: int, step: int, end: int) -> float:
        return CUDA_COMPLIER.CUDA_EXTENSION.compute_mse_loss(
            histogram, start, step, end
        )
