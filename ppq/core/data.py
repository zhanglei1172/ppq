...
class TensorMeta:
    def __init__(
        self, dtype: DataType, shape: List[int], tensor_name: str = None
    ) -> None:
    ...
    def parsing_from_torch_tensor(cls, torch_tensor: Tensor, name: str = None):
        if not isinstance(torch_tensor, Tensor):
            raise TypeError(
                f"Can not parse meta data for {type(torch_tensor)} instance, "
    ...
    def create_ndarray(self, fill_value: Any = 0):
        return ndarray(shape=self.shape, dtype=DataType.to_numpy(self.dtype)).fill(
            fill_value
        )
    ...
    def copy(self):
        if self.shape is not None:
            return TensorMeta(
                dtype=self.dtype, shape=self.shape.copy(), tensor_name=self.name
            )
        else:
...
def convert_primary_type_to_list(x: Union[int, float, list, tuple]) -> list:
    if isinstance(x, list) or isinstance(x, tuple):
        ...
        raise TypeError(
            f"input value {x}({type(x)}) can not be converted as python list."
        )
...
def convert_any_to_string(
    ...
    if isinstance(x, int) or isinstance(x, float):
        ...
        return "[{content}]".format(
            content="".join([convert_any_to_string(_) + "," for _ in x])
        )
    elif isinstance(x, torch.Tensor):
