...
op1 = graph.create_operation(
    ...
    inputs=[
        ...
        graph.create_variable(
            is_parameter=True, value=torch.rand(size=[128, 1024]).cuda() * 100
        ),
        graph.create_variable(
            is_parameter=True, value=torch.rand(size=[1024]).cuda() * 100
        ),
    ],
...
op2 = graph.create_operation(
    ...
    inputs=[
        op1.outputs[0],
        graph.create_variable(
            is_parameter=True, value=torch.rand(size=[1024, 128]).cuda()
        ),
...
op3 = graph.create_operation(
    ...
    inputs=[
        op2.outputs[0],
        graph.create_variable(
            is_parameter=True, value=torch.rand(size=[1024, 128]).cuda() * 500
        ),
        graph.create_variable(
            is_parameter=True, value=torch.rand(size=[1024]).cuda() * 0
        ),
    ],
    outputs=[graph.create_variable()],
)
...
LayerwiseEqualizationPass(
    iterations=1000, including_bias=True, including_act=True
).optimize(
    ...
    def __init__(self) -> None:
        ...
        with torch.no_grad():
            self.conv1 = torch.nn.Conv2d(
                in_channels=8, out_channels=32, kernel_size=3, stride=2, padding=1
            )
            ...
            self.convtranspose3 = torch.nn.ConvTranspose2d(
                in_channels=32, out_channels=8, groups=1, kernel_size=1
            )
...
model = MyModel().cuda()
dump_torch_to_onnx(
    model=model, onnx_export_file="model.onnx", input_shape=[1, 8, 96, 96]
)
...
LayerwiseEqualizationPass(
    iterations=10, including_bias=True, including_act=True
).optimize(
    graph=graph, dataloader=inputs, executor=executor, collate_fn=lambda x: x.cuda()
)
p_outputs = [executor.forward(inputs=t.cuda())[0].unsqueeze(0) for t in inputs]
...
model = MyModel().cuda()
dump_torch_to_onnx(
    model=model, onnx_export_file="model.onnx", input_shape=[1, 8, 16, 96, 96]
)
graph = load_onnx_graph(onnx_import_file="model.onnx")
...
class MyModel(torch.nn.Module):
    def __init__(self) -> None:
        ...
        with torch.no_grad():
            ...
            self.conv4 = torch.nn.ConvTranspose1d(
                in_channels=8, out_channels=32, kernel_size=3, stride=2, padding=1
            )
...
class MyModel(torch.nn.Module):
    def __init__(self) -> None:
        ...
        with torch.no_grad():
            ...
            self.conv4 = torch.nn.ConvTranspose3d(
                in_channels=8, out_channels=32, kernel_size=3, stride=2, padding=1
            )
...
model = MyModel().cuda()
dump_torch_to_onnx(
    model=model, onnx_export_file="model.onnx", input_shape=[1, 8, 8, 8, 8]
)
graph = load_onnx_graph(onnx_import_file="model.onnx")
