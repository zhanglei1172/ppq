...
for case in TORCH_TEST_CASES:
    try:
        ...
        session = onnxruntime.InferenceSession(
            "onnxruntime.onnx", providers=onnxruntime.get_available_providers()
        )
        onnxruntime_outputs = [
            session.run([output_name], {input_name: convert_any_to_numpy(sample)})
            for sample in dataset
        ]
        onnxruntime_outputs = [
            convert_any_to_torch_tensor(sample) for sample in onnxruntime_outputs
        ]
...
for case in TORCH_TEST_CASES:
    try:
        ...
        for name in quantized.inputs:
            input_name = name
        session = onnxruntime.InferenceSession(
            "onnx.onnx", providers=onnxruntime.get_available_providers()
        )
        onnxruntime_outputs = [
            session.run([output_name], {input_name: convert_any_to_numpy(sample)})
            for sample in dataset
