...
graph.append_operation(
    operation=Operation(name="op4", op_type="Sigmoid", attributes={})
)
...
graph.append_operation(
    operation=Operation(name="op7", op_type="Sigmoid", attributes={})
)
...
for var in graph.variables.copy():
    graph.insert_op_on_var(
        graph.create_operation(op_type="Test_1", attributes={}), var=var
    )
for var in graph.variables.copy():
    graph.insert_op_on_var(
        graph.create_operation(op_type="Test_2", attributes={}), var=var
    )
