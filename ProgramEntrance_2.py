...
class MyExporter(GraphExporter):
    def export(
        ...
        print(
            "This exporter does not export quantitative information to file, "
...
class MyInt8Quantizer(BaseQuantizer):
    def __init__(
        ...
        if power_of_2:
            self.policy = QuantizationPolicy(
                self.policy._policy + QuantizationProperty.POWER_OF_2
            )
    ...
    def init_quantize_config(self, operation: Operation) -> OperationQuantizationConfig:
        ...
        if operation.type in {"Conv", "ConvTranspose", "MatMul", "Gemm"}:
            if operation.num_of_input == 3:  # has bias
                ...
                OQC.input_quantization_config[
                    -1
                ].state = QuantizationStates.PASSIVE_INIT
            ...
            OQC.input_quantization_config[1].quant_max = (
                pow(2, self.num_of_bits - 1) - 1
            )
...
class MyOptimPass(QuantizationOptimizationPass):
    ...
    def optimize(self, graph: BaseGraph, **kwargs) -> None:
        ...
        patterns = processor.pattern_matching(
            patterns=["Relu", "Clip"], edges=[[0, 1]], exclusive=True
        )

        for relu, clip in patterns:
            print(f"Fuse {relu.name} and {clip.name}")
            if not isinstance(clip, QuantableOperation):
                continue
            if not isinstance(relu, QuantableOperation):
                continue
            relu.config.output_quantization_config[
                0
            ].dominated_by = clip.config.output_quantization_config[0]
            clip.config.input_quantization_config[
                0
            ].dominated_by = clip.config.output_quantization_config[0]
            clip.config.output_quantization_config[0].scale = torch.tensor(
                1 / 127
            ).cuda()
            clip.config.output_quantization_config[0].offset = torch.tensor(0.0).cuda()
            clip.config.output_quantization_config[
                0
...
with ENABLE_CUDA_KERNEL():
    ...
    graph = load_torch_model(
        model=model, sample=torch.zeros(size=[1, 3, 224, 224]).cuda()
    )

    quantizer = MyInt8Quantizer(
        graph=graph, per_channel=True, sym=True, power_of_2=False
    )
